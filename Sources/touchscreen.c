...
CalibrationSample calibrationSamples[CALIBRATION_SAMPLE_COUNT] = { { (SCREEN_WIDTH * 10) / 100, (SCREEN_HEIGHT * 10) / 100, 0, 0 }, { (SCREEN_WIDTH * 50)
    / 100, (SCREEN_HEIGHT * 90) / 100, 0, 0 }, { (SCREEN_WIDTH * 90) / 100, (SCREEN_HEIGHT * 50) / 100, 0, 0 }, { SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 0,
    0 } };
...
void touchScreenCalibrate()
{
    ...
    for (int i = 0; i < CALIBRATION_SAMPLE_COUNT; i++) {
        ...
        printf("(%d, %d) -> (%d, %d)\n", calibrationSamples[i].screenX, calibrationSamples[i].screenY, calibrationSamples[i].touchX,
            calibrationSamples[i].touchY);
    }
}
